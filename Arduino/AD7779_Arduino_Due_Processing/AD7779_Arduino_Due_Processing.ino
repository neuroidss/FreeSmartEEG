...
//Intel Edison
const int RESET_PIN                 =  8;//16;//D0
const int DRDY_PIN                  =  7;//18;//D2
const int ADCCS                     = 10;
const int START_PIN                 =  9;//D0
...
//  SPI.beginTransaction(SPISettings(speedMaximum, dataOrder, dataMode));
  SPI.beginTransaction(10,SPISettings(speedMaximum, dataOrder, dataMode));
  ui16Buffer = ((bregister & 0xff) * (1 << 8)) + (bcommand & 0xff);
//  SPI.transfer16(ui16Buffer);
  SPI.transfer16(10,ui16Buffer,SPI_CONTINUE);
...
//  ui16Buffer = SPI.transfer16(ui16Buffer);
  ui16Buffer = SPI.transfer16(10,ui16Buffer,SPI_CONTINUE);
  bdata=(ui16Buffer & 0xff);
//  bdata=SPI.transfer((byte)((0x80) + (byte)(bregister & 0xff)));
...
//  SPI.transferBuffer(aui8outRegisterBuffer,aui8inRegisterBuffer,sizeRegisterBuffer);
  SPI.transfer(10,aui8outRegisterBuffer,sizeRegisterBuffer,SPI_CONTINUE);
  bdata=aui8outRegisterBuffer[1];
  ui8RegisterCRC=aui8outRegisterBuffer[0];
////  SPI.transfer(bregister);
//  bdata=SPI.transfer(0);
//  bdata=SPI.transfer(0);
  SPI.endTransaction();
...
//    aui8inDataBuffer[iBuffer]=SPI.transfer(0x00);
    aui8inDataBuffer[iBuffer]=SPI.transfer(10,(byte)0,SPI_CONTINUE);
